...
class Laporan extends Component
{
    ...
    public function render()
    {
        $transaksis = null;
        if ($this->cari != null) {
            $transaksis = Transaction::where('jenis', 'Resep')
                ->where('no_transaksi', 'like', '%' . $this->cari . '%')
                ->orWhere('no_resep', 'like', '%' . $this->cari . '%')
                ->orWhere('pasien', 'like', '%' . $this->cari . '%')->paginate(15);
        } else {
            $transaksis = Transaction::where('jenis', 'Resep')->whereBetween('created_at', array($this->start, $this->end))->orderBy('created_at', 'DESC')->paginate(15);
        }
        $this->total = $transaksis->sum('total');
        return view('livewire.transaksi.resep.laporan')->with(compact('transaksis'));
    }
